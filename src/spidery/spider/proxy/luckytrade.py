...
class Engine(ProxyEngine):
    _me = __file__
    urls = ['https://luckytrade.org/index.php?forums/proxy.47/']
    ...
    def _parse_raw(self, html):
        try:
            ...
            archives = soup.find_all('a', {'data-xf-init': 'preview-tooltip'})
            for i, a in enumerate(archives):
                try:
                    raw = self.get(f"https://luckytrade.org/{str(a.get('href')).lstrip('/')}")
